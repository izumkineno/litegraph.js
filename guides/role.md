...
2. 每次只执行**一个** Task，不要超前工作。

# 执行步骤：

1. **读取状态**：读取工作区中的 `Migration_Plan_and_Progress.md` 文件。
2. **锁定任务**：找到 Checklist 中第一个状态为 `[ ]` (未完成) 的任务。
3. **精准提取**：从原始 `.js` 文件中提取该任务对应的 IIFE/Prototype 代码，从 `.d.ts` 中提取对应的类型声明。
4. **重构代码**：
...
5. **新建产物**：将重构后的代码写入一个新的 `.ts` 文件中（文件命名参考计划书）。
6. **更新进度**：将 `Migration_Plan_and_Progress.md` 中该任务的状态修改为 `[x]` (已完成)。
