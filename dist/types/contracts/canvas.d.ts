...
export interface GraphCanvasConstructorPort<TCanvas extends GraphCanvasLifecyclePort = GraphCanvasLifecyclePort> {
    new (...args: any[]): TCanvas;
}
export interface GraphCanvasCapturePort<TNode = unknown> {
    node_capturing_input: TNode | null;
}
export interface GraphCanvasViewportPort<TNode = unknown> extends GraphCanvasCapturePort<TNode> {
    ds: CanvasTransformPort;
}
export interface GraphCanvasWidgetPort<TGraph = unknown, TNode = unknown> extends GraphCanvasLifecyclePort<TGraph>, GraphCanvasViewportPort<TNode> {
