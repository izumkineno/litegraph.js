...
type SerializedLGraphNodeBase = Omit<Pick<LGraphNode, "id" | "type" | "pos" | "size" | "mode" | "inputs" | "outputs" | "title" | "properties">, "id" | "inputs" | "outputs">;
...
export type SerializedLGraphNode<TNode extends SerializedLGraphNodeLike = SerializedLGraphNodeLike> = {
    id: TNode["id"];
    type: TNode["type"];
    pos: TNode["pos"];
    size: TNode["size"];
...
export interface SerializedLGraphGroup extends Pick<LGraphGroup, "title" | "color"> {
    bounding: Vector4;
    font: string;
}
type SerializedLGraphBase = Pick<LGraph, "last_node_id" | "last_link_id" | "config">;
export type serializedLGraph<TNode = SerializedLGraphNode, TLink = [number, number, number, number, number, string], TGroup = SerializedLGraphGroup> = SerializedLGraphBase & {
    nodes: TNode[];
    links: TLink[];
    groups: TGroup[];
    version: number;
};
export type SerializedLGraph<TNode = SerializedLGraphNode, TLink = [number, number, number, number, number, string], TGroup = SerializedLGraphGroup> = serializedLGraph<TNode, TLink, TGroup>;
export {};
