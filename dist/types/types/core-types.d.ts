...
export type WidgetCallback<T extends IWidget = IWidget> = (this: T, value: T["value"], graphCanvas: GraphCanvasWidgetPort, node: LGraphNodeLike, pos: Vector2, event?: MouseEvent) => void;
export interface IWidget<TValue = any, TOptions = any> {
    ...
    draw?(ctx: CanvasRenderingContext2D, node: LGraphNodeLike, width: number, posY: number, height: number): void;
    /**
...
export interface IToggleWidget extends IWidget<boolean, {
    on?: string;
    off?: string;
}> {
    type: "toggle";
}
export interface ISliderWidget extends IWidget<number, {
    max: number;
    min: number;
}> {
    type: "slider";
}
export interface INumberWidget extends IWidget<number, {
    precision: number;
}> {
    type: "number";
}
export interface IComboWidget extends IWidget<string[], {
    values: string[] | ((widget: IComboWidget, node: LGraphNodeLike) => string[]);
}> {
    type: "combo";
}
...
export type ContextMenuEventListener = (value: ContextMenuItem, options: IContextMenuOptions, event: MouseEvent, parentMenu: ContextMenuLike | undefined, node: LGraphNodeLike) => boolean | void;
